...
A trainee, for whom you are responsible, must retrieve a list of users from a database but also
give the possibility to add a list using a form.

The main script consists of two parts:
- The list of users and their information
- The form to add a new user

The trainee starts in PHP programming. He does not indent or comment on his code. As a
tutor, you need to help him correct and run his code. A good indentation of the code is also
necessary.

Implementation steps:

1. Download the files from this address: http://wf3.axessweb.io/
evaluations/php-sql/evaluation_docs.zip

2. Deploy the index.php and connect.php files on your local server and do not forget to
import the SQL users-install.sql script. This will create a new database containing a
"users" table.

3. Help your trainee to debug his code, correct his mistakes and make it work.
...
if(!empty($_POST)) {
	...
	// If there is not an error, the data is sent to the database
	if(count($errors) == 0) { 
...
// Query to show the data already existants in the Database
